<template>
  <div class="container">
    <!-- <div class="input-container">
      ...
      <input v-model="phoneNumber" type="text" placeholder="Enter Your Mobile Number" />
    </div>
    <input placeholder="email" v-model="email"

    <button class="start-selling-button" @click="sendVerificationCode">Start Selling</button>
    ...
    <div id="recaptcha-container"></div> Placeholder for reCAPTCHA -->
<CgCard  :width=602>
    <form @submit.prevent="saveSupplier()">
      <h2 style="color: #bd3dc7;     background: var(--Gradient, linear-gradient(90deg, #0c73d4 1.04%, #e334f3 100%));
     -webkit-background-clip: text;
      color: transparent;">Sell online to 14 Cr+ customers at  0% Commission</h2>
      <h3>Become a seller</h3>
      <p style="color:gray;">Become a seller and grow your business across India</p>
     ...
     <p style="text-align: start; color:gray;">Enter pickup Address and make sure to provide steet and landmark</p>

        <div style="display: flex; flex-direction: column;">

     <label style="text-align: start;">state</label>

    <select aria-placeholder="select state"  class="text-select"   v-model="supplier.address. state" @change="onStateChange" v-if="states.length">
      <option v-for="state in states" :key="state.isoCode" :value="state.name">
        {{ state.name }}
      </option>
    </select>
  </div>
  <div v-if="cities.length" style="display: flex; flex-direction: column;" >
<label  style="text-align: start;" >city</label>
    <select  aria-placeholder="select city" class="text-select"  v-model="supplier.address.city" v-if="cities.length">
...
const supplier:  Omit<ISupplier, 'createdAt' | 'updatedAt'> = reactive({
    ...
    const onStateChange = () => {
      cities.value = City.getCitiesOfState(selectedCountry.value, supplier.address.state);
    };
